...
/**
     * @author Funkornaut
     * @notice Payroll optomized for Polygon
     * Assumptions:
...
contract PayrollGas is Ownable {
...
// State variables
   ...
   // uint64 public immutable i_destinationChainIdBase = 5790810961207155433;
    ...
    struct Employee { 
        ...
        uint256 payRate; //annual rate if sallary, hourly rate if hourly
        uint256 weeklyPay; // 0 if hourly, payRate/52 if salary
    ...
    struct PaymentSplit {
        ...
        //uint8 paySplitPercentage4; //Base 
    }
    ...
    //event EmployeePaid(address employee, uint amount); might need to add into loops when for graph data
    ...
    event SalarySet(address _employeeAddress, bool _isSalary);
...
// external & public view & pure functions
    ...
    function pay(uint64 _destinationChainSelector, address _employeeAddress, address _token, uint256 _amount) public returns (bytes32 messageId) {
        ...
        messageId = ccip.transferTokensPayLINK(_destinationChainSelector, _employeeAddress, _token, _amount);
    ...
    function payAllSallaryEmployees() public onlyAutomationOrOwner {
        for(uint i = 0; i < salariedEmployees.length; ++i) {
            ...
            uint256 weeklyPay = employees[employeeAddress].weeklyPay;
    ...
    function gas_payAllSallaryEmployees() public onlyAutomationOrOwner {
    ...
    function _transferToChain(uint64 _destinationChainSelector, address _receiver, address _token, uint256 _amount) internal returns (bytes32 messageId) {
        // @note what might be worth doing is totaling the amount of the entire payroll off-chain then have ccip be approved for that amount
        bnmToken.approve(address(ccip), _amount);

        bool success = bnmToken.transfer(address(ccip), _amount);
        require(success, "Token transfer failed");
        
        messageId = ccip.transferTokensPayLINK(_destinationChainSelector, _receiver, _token, _amount);
    ...
    function addEmployee(address _employeeAddress, bool _isSalary, uint256 _payRate) external onlyOwner {
        ...
        uint256 weeklyPayRate; 
        if(_isSalary == true) {
            weeklyPayRate = (_payRate / 52);
        } else {
            weeklyPayRate = 0;
        }
    ...
    function setEmployeeSalary(address _employeeAddress, bool _isSalary, uint256 _payRate) external onlyOwner {
        ...
        emit SalarySet(_employeeAddress, _isSalary);
    }
    ...
    function setPaymentSplits(address _employeeAddress, uint8[5] calldata _paySplitPercentages) public onlyEmployeeOrOwner {
        ...
        if (totalPercentage != 100) {revert PercentageMustEqual100();}
         ...
         PaymentSplit memory newSplit = PaymentSplit({
            ...
            //paySplitPercentage4: _paySplitPercentages[5]
        });
    ...
    function _calculateWeeklyPaymentSalary(address _employeeAddress) internal view returns (uint256 weeklyPay) {
        ...
        // PaymentSplit memory splits = employees[_employeeAddress].paymentSplits;
        //     //@note does grater than or != cost less gas?
        //     if (splits.paySplitPercentageNative != 0) {
