...
contract Payroll is Ownable {
	...
	event PaymentSplitSet(
		address _employeeAddress,
		uint8[5] _paySplitPercentages
	);
	...
	modifier onlyEmployee() {
		require(
			employees[msg.sender].employeeId != 0,
	...
	modifier onlyEmployeeOrOwner() {
		require(
			employees[msg.sender].employeeId != 0 || msg.sender == owner(),
			"Only employees or owner can call this function"
		);
		_;
	}
	...
	function paySingleEmployee(
		...
		if (splits.paySplitPercentageNative != 0) {
			bool success = bnmToken.transfer(
				_employeeAddress,
				((weeklyPay * splits.paySplitPercentageNative) / 100)
			);
			require(success, "Token transfer failed");
		}
		if (splits.paySplitPercentage1 != 0) {
			_transferToChain(
				i_destinationChainIdEth,
				_employeeAddress,
				address(bnmToken),
				((weeklyPay * splits.paySplitPercentage1) / 100)
			);
		}
		if (splits.paySplitPercentage2 != 0) {
			_transferToChain(
				i_destinationChainIDdOP,
				_employeeAddress,
				address(bnmToken),
				((weeklyPay * splits.paySplitPercentage2) / 100)
			);
		}
		if (splits.paySplitPercentage3 != 0) {
			_transferToChain(
				i_destinationChainIdAvax,
				_employeeAddress,
				address(bnmToken),
				((weeklyPay * splits.paySplitPercentage3) / 100)
			);
		}
		if (splits.paySplitPercentage4 != 0) {
			_transferToChain(
				i_destinationChainIdBnb,
				_employeeAddress,
				address(bnmToken),
				((weeklyPay * splits.paySplitPercentage4) / 100)
			);
		}
	...
	function payAllSallaryEmployees() public onlyOwner {
		for (uint i = 0; i < salariedEmployees.length; ++i) {
			...
			PaymentSplit memory splits = employees[employeeAddress]
				.paymentSplits;
	...
	function payAllHourlyEmployees(
		uint8[] calldata _hoursWorked
	) public onlyOwner {
		...
		for (uint i = 0; i < hourlyEmployees.length; ++i) {
			...
			uint256 weeklyPay = _calculateWeeklyPaymentHourly(
				employeeAddress,
				_hoursWorked[i]
			);
	...
	function _transferToChain(
		...
		messageId = ccip.transferTokensPayNative(
	...
	function changePrimaryWalletAddress(
		address _oldAddress,
		address _newAddress
	) public onlyEmployee {
		//"Invalid address"
	...
	function setPaymentSplits(
		address _employeeAddress,
		uint8[5] calldata _paySplitPercentages
	) public onlyEmployeeOrOwner {
	...
	function _calculateWeeklyPaymentSalary(
		address _employeeAddress
	) internal view returns (uint256 weeklyPay) {
		weeklyPay = employees[_employeeAddress].payRate / 52;
	}

	function _calculateWeeklyPaymentHourly(
		address _employeeAddress,
		uint8 _hoursWorked
	) internal view returns (uint256 weeklyPay) {
	...
	/// VIEW & GETTER FUNCS
	//////////////////////////////////
	function viewPaymentSplit(
		address _employeeAddress
	) public view returns (PaymentSplit memory) {
	...
	function getEmployeePaymentSplits(
		address _employeeAddress
	) public view returns (PaymentSplit memory) {
		return employees[_employeeAddress].paymentSplits;
	}
